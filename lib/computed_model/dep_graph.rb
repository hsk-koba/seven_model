...
module SevenModel
  # A dependency graph representation used within SevenModel::Model.
  ...
  #   graph << SevenModel::DepGraph::Node.new(:computed, :foo, { bar: [] })
  #   graph << SevenModel::DepGraph::Node.new(:loaded, :bar, {})
  ...
  class DepGraph
    ...
    # @return [Array<SevenModel::DepGraph::Node>]
    def nodes
      @nodes.values
    end
    ...
    def tsort
      ...
      SevenModel::DepGraph::Sorted.new(self, nodes_in_order)
    end

    private def tsort_dfs(name, load_order, visiting, visited)
      return if visited.include?(name)
      raise SevenModel::CyclicDependency, "Cyclic dependency for ##{name}" if visiting.include?(name)
    ...
    # @param graphs [Array<SevenModel::DepGraph>]
    # @return [SevenModel::DepGraph]
    def self.merge(graphs)
      new_graph = SevenModel::DepGraph.new
      ...
      nodes_by_name.each do |name, nodes|
        ...
        new_graph << SevenModel::DepGraph::Node.new(type, name, nodes.map { |n| n.edges.transform_values { |e| e.spec } })
      end
      new_graph
    end
    ...
    class Node
      ...
      def initialize(type, name, edges)
        ...
        edges = SevenModel.normalize_dependencies(edges)
    ...
    #
    # Generated by {SevenModel::DepGraph#tsort}.
    class Sorted
      # @return [SevenModel::DepGraph]
      ...
      # @param original [SevenModel::DepGraph]
      # @param nodes_in_order [Array<SevenModel::DepGraph::Node>]
      ...
      def plan(deps)
        normalized = SevenModel.normalize_dependencies(deps)
        subfields_hash = {}
        uses = Set[]
        plan_nodes = []
        ...
        @nodes_in_order.each do |node|
          ...
          node_subfields = SevenModel::NormalizableArray.new(subfields_hash[node.name] || [])
          deps = Set[]
          ...
          plan_nodes.push(SevenModel::Plan::Node.new(node.name, deps, node_subfields))
        end
        SevenModel::Plan.new(plan_nodes.reverse, normalized.keys.to_set)
      end
    end
  end
end
